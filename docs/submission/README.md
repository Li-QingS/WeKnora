...
- Wiki 页面生成热运行缓存读取率：`20.95% → 21.49%`，增加 0.54 个百分点。
- Wiki 引用抽取热运行缓存读取率：`9.83% → 15.34%`，增加 5.51 个百分点。
- 全部 Wiki 热运行缓存读取率：`16.46% → 24.97%`，增加 8.51 个百分点；其中包含候选抽取的隐式缓存收益，直接效果应以前两项分阶段数据为准。
