...
- [ ] 真实 GitHub Actions pass run 完成并保存 run URL、日志、PNG。
- [ ] 人为降低 Recall 的真实 GitHub Actions fail run 完成，输出含 delta/threshold，并保存 run URL、日志、PNG。
- [ ] 分支最终恢复正常 pass 场景，没有故意退化数据。
- [ ] 敏感信息扫描无命中。
- [ ] 相关测试、构建和格式检查通过。
