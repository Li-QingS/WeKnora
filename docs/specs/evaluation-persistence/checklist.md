...
- [ ] `GET /evaluation/runs` 已注册，Viewer 可读；现有 `POST /evaluation` 与 `GET /evaluation?task_id=` 请求/响应保持不变。（验证：路由与 handler 测试 + 请求回归）
...
- [ ] 场景 1（正常完成）：Admin 发起评测 → 立即查库出现 pending → 轮询接口看到 running 且 finished 递增 → 完成后数据库为 success、12 项指标齐全、finished == total、config_snapshot/config_hash 完整；重启服务后同一 task_id 仍返回同样结果。（验证：真实服务完整跑一遍）
- [ ] 场景 2（运行中重启）：评测运行中重启服务 → 启动后同 task_id 返回 interrupted 与 finished_at，已写入进度保留 → 再次发起同配置评测生成新记录，旧记录不被覆盖。（验证：真实服务完整跑一遍）
- [ ] 场景 3（失败与隔离）：用不存在的模型发起评测 → 最终为 failed 且 err_msg 可定位；B 租户查询/列表均看不到 A 租户的记录。（验证：真实服务完整跑一遍）

## 验收记录（2026-08-30）

- `go build ./...`、`go vet ./internal/...`、`go test ./internal/...` 均通过。
- 本工作包相关单测在 `-race` 下通过：`TestEvaluationRun*`、`TestEvaluationPersist*`。
- 未完成项：AC10 本地基准、AC11 存储故障注入、真实服务端到端场景、`golangci-lint`（本机未安装）、新列表端点的 HTTP/权限实测。
