...
func formatKnowledgeBaseList(kbInfos []*KnowledgeBaseInfo) string {
	...
	// Deterministic ordering keeps the prompt bytes stable across runs so
	// provider prompt-prefix caches stay valid.
...
func formatSkillsMetadata(skillsMetadata []*skills.SkillMetadata, shellExecEnabled bool) string {
	...
	// Deterministic ordering keeps the prompt bytes stable across runs.
