...
func TestExitCode(t *testing.T) {
	cases := []struct {
		...
		{"eval.regression", NewError(CodeEvalRegression, "regression"), 2},
		{"eval.config_error", NewError(CodeEvalConfigError, "bad config"), 3},
		{"eval.service_unavailable", NewError(CodeEvalServiceUnavailable, "service down"), 4},
		{"eval.run_failed", NewError(CodeEvalRunFailed, "run failed"), 5},
