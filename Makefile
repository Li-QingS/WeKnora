...
build-prod:
	...
	GIT_DIRTY=$$(./scripts/get_version.sh env | sed -n 's/^GIT_DIRTY=//p'); \
	CGO_ENABLED=1 \
	CGO_CFLAGS="-Wno-deprecated-declarations" \
	CGO_LDFLAGS="$$(if [ "$$(uname)" = 'Darwin' ]; then echo '-Wl,-no_warn_duplicate_libraries'; fi)" \
	BUILD_TIME=$${BUILD_TIME:-unknown}; \
	GO_VERSION=$${GO_VERSION:-unknown}; \
	LDFLAGS="-X 'github.com/Tencent/WeKnora/internal/handler.Version=$$VERSION' -X 'github.com/Tencent/WeKnora/internal/handler.Edition=standard' -X 'github.com/Tencent/WeKnora/internal/handler.CommitID=$$COMMIT_ID' -X 'github.com/Tencent/WeKnora/internal/handler.BuildTime=$$BUILD_TIME' -X 'github.com/Tencent/WeKnora/internal/handler.GoVersion=$$GO_VERSION' -X 'github.com/Tencent/WeKnora/internal/buildinfo.Version=$$VERSION' -X 'github.com/Tencent/WeKnora/internal/buildinfo.CommitID=$$COMMIT_ID' -X 'github.com/Tencent/WeKnora/internal/buildinfo.GitDirty=$$GIT_DIRTY' -X 'github.com/Tencent/WeKnora/internal/buildinfo.GoVersion=$$GO_VERSION' -X 'google.golang.org/protobuf/reflect/protoregistry.conflictPolicy=warn'"; \
